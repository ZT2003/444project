...
export class HomePage implements OnInit {
  ...
  async confirm() {
    ...
    if (
      this.summary.type != undefined &&
      this.summary.title != undefined &&
      this.summary.topic != undefined
    ) {
      let flag = true;
      if (this.summary.type === 'book') {
        for (let i = 0; i < this.summary.chapters.length; ++i) {
          if (
            this.summary.chapters[i].chapter == null ||
            this.summary.chapters[i].summary == ''
          )
            flag = false;
        }
        if (flag) this.modal.dismiss(this.summary, 'confirm');
  ...
  removeChapter(i) {
    ...
    if (this.summary.chapters.length == 1) this.remove = true;
  }
  onSelectionChange() {
    const selectedCount = this.filteredSummaries.filter(
      (s) => s.selected
    ).length;
    this.isCombineButtonVisible = selectedCount > 1;
  }

  combineSelected() {
    const selectedSummaries = this.filteredSummaries.filter((s) => s.selected);
    this.combinedDraft.summary = '';
    selectedSummaries.forEach((summary, index) => {
      this.combinedDraft.summary += `${index + 1}. ${summary.title}\n${
        summary.summary
      }\n\n`;
    });
    this.createDraft();
  ...
  createDraft() {
    this.fb.addDraft(this.combinedDraft).then(() => {
      alert('Draft created successfully');
      this.filteredSummaries.forEach((summary) => (summary.selected = false));
      this.isCombineButtonVisible = false;
